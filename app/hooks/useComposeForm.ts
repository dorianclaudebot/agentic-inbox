...
import { fileToBase64Payload, validateComposeFiles } from "~/lib/attachments";
import {
  useDeleteEmail,
  useForwardEmail,
  useReplyToEmail,
...
import { useUIStore, type ComposeMode } from "~/hooks/useUIStore";
...
function getComposeFormTitle(mode: ComposeMode, isDraftEdit: boolean): string {
...
export function useComposeForm(mailboxId?: string, _folder?: string) {
  ...
  const formTitle = useMemo(
    () => getComposeFormTitle(composeOptions.mode, isDraftEdit),
  ...
  function addFiles(fileList: FileList) {
    const result = validateComposeFiles(files, Array.from(fileList));
  ...
  const handleSend = async (e: FormEvent, onClose: () => void) => {
    ...
    if (files.length > 0) {
      const check = validateComposeFiles([], files);
      if (check.error) {
    ...
    const from =
      fromName && fromName !== currentMailbox.email
        ...
        : currentMailbox.email;
    const attachments =
      files.length > 0 ? await Promise.all(files.map(fileToBase64Payload)) : undefined;
    const emailData = {
