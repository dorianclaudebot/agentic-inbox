...
function isMovedListRow(email: Email, id: string, threadId?: string | null): boolean {
  if (threadId && email.thread_id === threadId) return true;
  return email.id === id;
}
  ...
  return useMutation({
    ...
    onMutate: async ({ mailboxId, id, folderId, threadId, sourceFolderId }) => {
      ...
      for (const [key, cached] of listQueries) {
        if (!cached?.emails) continue;
        const params = key[2];
        if (typeof params !== "object" || params === null || !("folder" in params)) {
          continue;
        }
        const folder = params.folder;
        if (typeof folder !== "string" || folder === folderId) continue;
        if (sourceFolderId && folder !== sourceFolderId) continue;
        const nextEmails = cached.emails.filter((email) => !isMovedListRow(email, id, threadId));
        if (nextEmails.length === cached.emails.length) continue;
        qc.setQueryData(key, {
          ...cached,
          emails: nextEmails,
          totalCount: Math.max(0, cached.totalCount - (cached.emails.length - nextEmails.length)),
        });
      }
...
export function useDraftReply() {
  ...
  return useMutation({
    mutationFn: ({ mailboxId, emailId }: { mailboxId: string; emailId: string }) =>
      api.draftReply(mailboxId, emailId),
    onSuccess: (_data, { mailboxId }) => invalidate(mailboxId),
  });
}
