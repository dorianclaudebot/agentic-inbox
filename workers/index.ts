...
app.post("/api/v1/mailboxes/:mailboxId/push/subscribe", handlePushSubscribe);
app.delete("/api/v1/mailboxes/:mailboxId/push/subscribe", handlePushUnsubscribe);
...
app.get(
  ...
  async (c: AppContext) => {
    ...
    const disposition =
      wantInline && isBrowserInlinePreviewable(attachment.mimetype, attachment.filename)
        ? "inline"
        : "attachment";
    headers.set(
      "Content-Disposition",
...
async function receiveEmail(
  ...
  if (!parsedEmail.to?.length) throw new Error("received email with empty to");

  const allowedAddresses = ((env.EMAIL_ADDRESSES ?? []) as string[]).map((a) => a.toLowerCase());
  const allRecipients = mailboxEmails(parsedEmail.to);
  const ccRecipients = mailboxEmails(parsedEmail.cc);
  const bccRecipients = mailboxEmails(parsedEmail.bcc);
  ...
  await stub.createEmail(
    Folders.INBOX,
    {
      ...
      sender: (mailboxEmail(parsedEmail.from) || "").toLowerCase(),
