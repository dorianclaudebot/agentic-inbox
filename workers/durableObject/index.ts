...
export class MailboxDO extends DurableObject<Env> {
  ...
  async getEmails(options: GetEmailsOptions = {}) {
    ...
    return this.#attachLabelsToEmails(
      result.map((email) => ({
        ...email,
        read: !!email.read,
  ...
  async updateLabel(id: string, patch: { name?: string; color?: string }) {
    const current = this.db.select().from(schema.labels).where(eq(schema.labels.id, id)).get();
  ...
  async applyLabel(emailId: string, labelId: string) {
    ...
    const already = this.db
      ...
      .from(schema.emailLabels)
      .where(
        and(eq(schema.emailLabels.email_id, emailId), eq(schema.emailLabels.label_id, labelId)),
      )
      .get();
    if (!already) {
  ...
  async getEmailsByLabel(labelId: string, options: { page?: number; limit?: number } = {}) {
    ...
    return {
      emails: this.#attachLabelsToEmails(
        emails.map((email) => ({
          ...email,
          read: !!email.read,
          starred: !!email.starred,
  ...
  async savePushSubscription(subscription: StoredPushSubscription) {
    upsertPushSubscription(this.ctx.storage.sql, subscription);
  }

  async deletePushSubscription(endpoint: string) {
    deletePushSubscription(this.ctx.storage.sql, endpoint);
  }

  async listPushSubscriptions(): Promise<StoredPushSubscription[]> {
    return listPushSubscriptions(this.ctx.storage.sql);
  }
}
